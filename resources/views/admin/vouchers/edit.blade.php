    ...
    <form action="{{ route('admin.vouchers.update', $voucher->id) }}" method="POST" class="bg-white rounded-lg shadow-md p-6 space-y-6">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" name="starts_at" value="{{ old('starts_at', $voucher->starts_at ? \Carbon\Carbon::parse($voucher->starts_at)->format('Y-m-d') : '') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Hạn sử dụng</label>
                <input type="date" name="ends_at" value="{{ old('ends_at', $voucher->ends_at ? \Carbon\Carbon::parse($voucher->ends_at)->format('Y-m-d') : '') }}" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
            </div>
        </div>
