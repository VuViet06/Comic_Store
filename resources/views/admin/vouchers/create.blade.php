...
@section('content')
<div class="max-w-2xl">
    <form action="{{ route('admin.vouchers.store') }}" method="POST" class="bg-white rounded-lg shadow-md p-6 space-y-6">
        @csrf
        
        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Mã voucher <span class="text-red-500">*</span></label>
            <input type="text" name="code" value="{{ old('code') }}" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 font-mono uppercase @error('code') border-red-500 @enderror">
            @error('code')
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="value" value="{{ old('value') }}" required min="0" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('value') border-red-500 @enderror">
                @error('value')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" name="min_order_amount" value="{{ old('min_order_amount', 0) }}" min="0" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Giảm tối đa (VNĐ)</label>
                <input type="number" name="max_discount" value="{{ old('max_discount') }}" min="0" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-4">
            ...
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Tạo mã</button>
        </div>
    </form>
</div>
@endsection
