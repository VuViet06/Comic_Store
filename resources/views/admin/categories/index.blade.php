...
<div class="space-y-6">
    <div class="flex justify-between items-center gap-4">
        <form method="GET" action="{{ route('admin.categories.index') }}" class="flex gap-2">
            ...
            @if(request('search'))
                <a href="{{ route('admin.categories.index') }}" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300">
                    Hoàn tác
                </a>
            @endif
        </form>
            ...
            <tbody class="divide-y">
                @forelse($categories as $category)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-sm">{{ $category->comics_count ?? 0 }}</td>
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('admin.categories.destroy', $category->id) }}" method="POST" class="inline" onsubmit="return confirm('Bạn chắc chắn muốn xóa danh mục này ?')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-gray-500">Chưa có danh mục nào</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
