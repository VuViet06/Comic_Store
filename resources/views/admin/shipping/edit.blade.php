...
@section('title', 'Sửa đối tác vận chuyển')
@section('page-title', 'Sửa đối tác: ' . $partner->name)

@section('content')
<div class="max-w-2xl">
    <form action="{{ route('admin.shipping.update', $partner->id) }}" method="POST" class="bg-white rounded-lg shadow-md p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <input type="text" name="code" value="{{ old('code', $partner->code) }}" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 font-mono uppercase">
            @error('code')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div>
            ...
            <input type="number" name="shipping_fee" value="{{ old('shipping_fee', $partner->shipping_fee ?? 0) }}" min="0"
                class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Thời gian giao hàng dự kiến</label>
            <input type="text" name="estimated_days" value="{{ old('estimated_days', $partner->estimated_days ?? '') }}" placeholder="VD: 2-3 ngày"
                class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">
        </div>

        <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">Mô tả</label>
            <textarea name="description" rows="3" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">{{ old('description', $partner->description ?? '') }}</textarea>
        </div>

        <div>
            <label class="flex items-center">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $partner->is_active ?? true) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600">
