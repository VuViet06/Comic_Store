...
@section('title', 'Sửa NXB')
@section('page-title', 'Sửa NXB: ' . $publisher->name)

@section('content')
<div class="max-w-2xl">
    <form action="{{ route('admin.publishers.update', $publisher->id) }}" method="POST" class="bg-white rounded-lg shadow-md p-6 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500">{{ old('description', $publisher->description) }}</textarea>
        </div>
